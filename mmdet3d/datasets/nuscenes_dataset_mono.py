...
class NuScenesDatasetMono(Custom3DDataset):
        ...
        if self.modality['use_camera']:
            if self.img_info_prototype == 'mmcv':
                ...
                if not self.test_mode:
                    cam_anno_infos = dict()
                    for cam_type, cam_info in info["cams"].items():
                        cam_anno_infos.update({
                            cam_type: self.get_mono_anno_info(
                                cam_type=cam_type,
    ...
    def get_mono_anno_info_debug(self, cam_type, cam_info, foldername=None):
        ...
        for i, ann in enumerate(mono_ann_info):
            ...
            if ann.get("iscrowd", False):
                ...
                vis_tokens.append(ann["visibility_token"])
                min_corners.append(ann["min_depth_corner"])

        if gt_bboxes:
            gt_bboxes = np.array(gt_bboxes, dtype=np.float32)
            gt_labels = np.array(gt_labels, dtype=np.int64)
            depths = np.array(depths, dtype=np.float32)
            min_corners = np.array(min_corners, dtype=np.float32)
        else:
            gt_bboxes = np.zeros((0, 4), dtype=np.float32)
            gt_labels = np.array([], dtype=np.int64)
            depths = np.array([], dtype=np.float32)
            min_corners = np.array([], dtype=np.float32)
...
@DATASETS.register_module()
class NuScenesDatasetMonoForStereo(NuScenesDatasetMono):
    def __init__(self, stereo=False, **kwargs):
